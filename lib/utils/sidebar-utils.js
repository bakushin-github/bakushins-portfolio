...
export function generateSocialMetadata(post, slug) {
  ...
  const description = post.excerpt || `${post.title}の詳細ページです。都市開発に従事している研究者による考察記事です。`;
...
export function generateJsonLd(post, slug) {
  ...
  return {
    ...
    author: {
      '@type': 'Person',
      name: 'バクシン',
      description: '都市開発に従事している研究者です。ユーザー目線の開発について考察した記事を投稿しております。',
    ...
    keywords: post.categories?.nodes?.map(cat => cat.name).join(', ') || 'ブログ, 都市開発, ユーザー目線',
  };
}
...
export function getAuthorData() {
  return {
    name: "バクシン",
    description: "都市開発に従事している研究者です。ユーザー目線の開発について考察した記事を投稿しております。WordPress、ShopifyなどのCMS、React（Next.js）で制作しています。",
...
export function generateRssData(posts) {
  ...
  return {
    title: 'バクシンブログ',
    description: '都市開発に従事している研究者による、ユーザー目線の開発について考察したブログです。',
